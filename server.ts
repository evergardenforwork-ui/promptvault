...
function bookmarkToDb(data: any, userId: string) {
  return {
    ...
    title: data.title || "",
    url: data.url || "",
    favicon: data.favicon || data.faviconUrl || data.favicon_url || null,
    image: data.image || null,
    description: data.description || null,
    category: data.category || "default",
    folder: data.folder || data.folderId || data.folder_id || "Общее",
    tags: data.tags || [],
    is_public: data.isPublic ?? true,
    workspace_id: data.workspaceId || null,
    author_name: data.authorName || "",
...
function bookmarkFromDb(row: any, isFavorite = false) {
  return {
    ...
    title: row.title,
    url: row.url || "",
    favicon: row.favicon || row.favicon_url || null,
    image: row.image || null,
    description: row.description || "",
    category: row.category || "default",
    folder: row.folder || row.folder_id || "Общее",
    tags: Array.isArray(row.tags) ? row.tags : (typeof row.tags === 'string' ? JSON.parse(row.tags || '[]') : []),
    isPublic: Boolean(row.is_public),
    workspaceId: row.workspace_id || null,
    authorName: row.author_name || "",
