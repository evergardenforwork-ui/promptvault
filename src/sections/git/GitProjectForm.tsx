  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* Header */}
          <div className="sticky top-0 z-10 bg-zinc-950/95 backdrop-blur border-b border-zinc-800/80 px-6 py-4 flex items-center justify-between">
            <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={onClose} className="p-2 hover:bg-zinc-800 rounded-xl text-zinc-500 hover:text-white transition-all cursor-pointer">
                <X size={20} />
              </button>
            </div>
          </div>
          ...
          <div className="p-6 space-y-5">
            ...
            {/* Image Upload */}
            <div
              className="relative w-full aspect-video rounded-2xl border-2 border-dashed border-zinc-800 hover:border-emerald-700 overflow-hidden cursor-pointer transition-all group"
            ...
            >
              {form.image ? (
                ...
                <div className="flex flex-col items-center justify-center h-full gap-2 text-zinc-600 group-hover:text-emerald-500 transition-colors">
                  <ImageIcon size={28} />
                  <span className="text-sm font-semibold">Скриншот / Баннер</span>
            ...
            <div>
              ...
              <input
                ...
                className={cn(
                  'w-full bg-zinc-900 border rounded-xl px-4 py-3 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-emerald-500 transition-colors',
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Категория</label>
                <select
                  value={form.category}
                  onChange={(e) => setField('category', e.target.value as GitProjectCategory)}
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-emerald-500 cursor-pointer"
                >
                  {GIT_CATEGORY_OPTIONS.map(o => (
                    <option key={o.value} value={o.value} className="bg-zinc-900">{o.emoji} {o.label}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Стоимость</label>
                  ...
                  {GIT_PRICING_OPTIONS.map(o => (
                    <button
                      ...
                      className={cn(
                        ...
                        form.pricing === o.value ? o.color : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:text-zinc-300'
                      )}
                    >
                      {o.label}
                    </button>
            ...
            <div>
              <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">
                Краткое описание (слоган) <span className="text-rose-400">*</span>
              </label>
              <textarea
                value={form.summary}
                ...
                rows={2}
                className={cn(
                  'w-full bg-zinc-900 border rounded-xl px-4 py-3 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-emerald-500 transition-colors resize-none',
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-emerald-500 transition-colors"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">
            ...
            <div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 flex flex-wrap gap-2 focus-within:border-emerald-500 transition-colors min-h-[52px]">
                {form.tags.map(tag => (
                  <span key={tag} className="flex items-center gap-1 px-2.5 py-1 bg-emerald-950/80 text-emerald-300 border border-emerald-800/40 text-xs font-semibold rounded-full">
                    #{tag}
                ...
                <input
                  ...
                  className="flex-1 min-w-[120px] bg-transparent text-white text-xs placeholder-zinc-600 outline-none"
                />
              </div>
            </div>
            ...
            {[
              ...
              <div key={key} className="border border-zinc-800/60 rounded-2xl overflow-hidden">
                <button
                  type="button"
                    ...
                    >
                      <div className="p-4">
                        <textarea
                          ...
                          rows={rows}
                          className={cn(
                            'w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-emerald-500 transition-colors resize-y',
            ...
            {/* Public toggle */}
            <div className="flex items-center justify-between p-4 bg-zinc-900/50 rounded-2xl border border-zinc-800/60">
              <div>
                <p className="text-sm font-bold text-zinc-300">Публичный проект</p>
              ...
              <button
                ...
                className={cn(
                  ...
                  form.isPublic ? 'bg-emerald-500' : 'bg-zinc-700'
                )}
              >
                <span className={cn(
          ...
          {/* Footer */}
          <div className="sticky bottom-0 bg-zinc-950/95 backdrop-blur border-t border-zinc-800/80 px-6 py-4 flex items-center justify-between">
            <button onClick={onClose} className="px-4 py-2 text-sm text-zinc-500 hover:text-white transition-colors cursor-pointer">
              Отмена
            </button>
            <button
              onClick={handleSave}
