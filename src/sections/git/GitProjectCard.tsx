...
export default function GitProjectCard({
  ...
  if (viewMode === 'list') {
    return (
      <motion.div
        ...
        whileHover={{ y: -2 }}
        onClick={onView}
        className="group relative bg-zinc-900/90 border border-zinc-800 hover:border-emerald-800/60 rounded-2xl p-4 cursor-pointer transition-all duration-200 hover:shadow-[0_12px_40px_-12px_rgba(52,211,153,0.12)] flex items-center gap-4"
      >
        <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 border border-zinc-800">
          {project.image ? (
            <img src={project.image} alt={project.title} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full bg-emerald-950/60 flex items-center justify-center text-2xl">
              {categoryOpt?.emoji ?? ''}
            </div>
          )}
        </div>
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="px-2 py-0.5 bg-emerald-950/80 text-emerald-300 border border-emerald-800/40 text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-1 shrink-0">
          ...
          {project.summary && <p className="text-xs text-zinc-400 truncate">{highlight(project.summary)}</p>}
        ...
        <div className="flex items-center gap-2 shrink-0" onClick={stopProp}>
          {project.githubUrl && (
            <a href={project.githubUrl} target="_blank" rel="noopener noreferrer"
              className="p-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white transition-all" title="GitHub">
              <Github size={15} />
            </a>
          )}
          {project.demoUrl && (
            <a href={project.demoUrl} target="_blank" rel="noopener noreferrer"
              className="p-2 rounded-xl bg-zinc-800 hover:bg-emerald-900/60 text-zinc-400 hover:text-emerald-300 transition-all" title="Demo">
              <Globe size={15} />
            </a>
          )}
          {effectiveUser && (
            <button onClick={(e) => { e.stopPropagation(); onToggleFavorite(); }}
              className={cn('p-2 rounded-xl transition-all duration-200 cursor-pointer',
                project.isFavorite ? 'bg-emerald-500 text-white' : 'bg-zinc-800 text-zinc-500 hover:text-white hover:bg-zinc-700')}>
              <Star size={14} fill={project.isFavorite ? 'currentColor' : 'none'} />
            </button>
          )}
        </div>
        <div className="hidden md:flex items-center gap-4 text-[11px] text-zinc-500 shrink-0">
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      onClick={onView}
      className="group relative bg-zinc-900/90 border border-zinc-800 hover:border-emerald-800/60 rounded-3xl cursor-pointer transition-all duration-200 hover:shadow-[0_20px_50px_-12px_rgba(52,211,153,0.15)] flex flex-col overflow-hidden"
    >
        ...
        {effectiveUser && (
          <button onClick={(e) => { e.stopPropagation(); onToggleFavorite(); }}
            className={cn('absolute top-3 right-3 p-2 rounded-full backdrop-blur-md transition-all duration-200 z-10 cursor-pointer',
              project.isFavorite ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'bg-black/40 text-zinc-300 hover:bg-black/60 hover:text-white')}>
            <Star size={14} fill={project.isFavorite ? 'currentColor' : 'none'} />
          </button>
        )}
      </div>
      <div className="flex flex-col flex-1 p-5 gap-3">
        <span className="px-2.5 py-1 bg-emerald-950/80 text-emerald-300 border border-emerald-800/40 text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-1.5 w-fit">
          <span>{categoryOpt?.emoji}</span>
          <span>{categoryOpt?.label ?? project.category}</span>
        </span>
        ...
        {project.summary && (
          <p className="text-sm text-zinc-400 line-clamp-2 leading-relaxed">{highlight(project.summary)}</p>
        )}
        ...
        {(project.githubUrl || project.demoUrl) && (
          <div className="flex gap-2" onClick={stopProp}>
            {project.githubUrl && (
              <a href={project.githubUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white text-xs font-semibold transition-all">
                <Github size={13} />GitHub
              </a>
            )}
            {project.demoUrl && (
              <a href={project.demoUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-900/60 hover:bg-emerald-800/60 text-emerald-300 hover:text-emerald-200 text-xs font-semibold border border-emerald-800/40 transition-all">
