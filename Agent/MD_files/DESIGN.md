...
- **SpaceFileTree**: Стилизация под VS Code дерево файлов. Тонкие отступы, иконки папок/файлов, выделение активного файла через `bg-surface/50`, чекбоксы для выделения.
- **SpaceFilePreview**: Область просмотра Markdown/кода с breadcrumb-навигацией в верхней части.
- **SpaceContextMenu**: Glassmorphism меню (`bg-surface/80 backdrop-blur-md border border-border/50`) по клику правой кнопкой мыши.
- **SpaceSelectionBar**: Плавающая панель снизу для выбранных файлов, анимированная с помощью пружинных (spring) анимаций (motion).
