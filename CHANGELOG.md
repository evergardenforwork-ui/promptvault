...
| `HEAD` | 2026-08-27 | **👑 Universal Ownership Auto-Claim, 1-Click Material Transfer & Canonical Schema.sql** | Внедрена умная автоматическая привязка владения при импорте ZIP-архивов (`Auto-Claim Ownership`): при переносе материалов с локалки в онлайн (или между пользователями) все карточки мгновенно привязываются к профилю импортера без ошибок Foreign Key. Добавлена кнопка `⚡ Привязать всё ко мне` (`POST /api/admin/claim-all`) с безопасной миграцией `user_favorites` без коллизий составных ключей. В окне входа добавлена кнопка быстрого заполнения локального администратора (`admin123`). Создан единый мастер-скрипт `scripts/schema.sql`, удалены 7 фрагментированных `.sql` файлов, а `manageUsers.ts` переведен на Dual-Engine. |
| `d0919ae` | 2026-08-26 | **🔍 Hierarchical Subfolder Tree Resolution, 5-Hub Favorites Parity & Gemini 3.1 Docs Sync** | Исправлен баг обнаружения и выбора вложенных под-папок в `BookmarkForm` (построение полного дерева через `buildAllFoldersMap`, неразрывные отступы `\u00A0` в select, глобальная синхронизация `saveCustomFolders`). Внедрен фильтр избранного ⭐ в `SkillsSection` и `useSkillFilters` (100% паритет фильтрации избранного во всех 5 хабах). Исправлен запрос `maybeSingle()` в Supabase `toggleFavorite`. Проведен глобальный аудит кода и синхронизация всей документации (PRD, PLAN, SCHEMA, ARCHITECTURE, GEMINI, AGENTS, CLAUDE) с моделью Gemini 3.1 Flash-Lite. |
| `6b1d2e9` | 2026-08-26 | **🎛️ Dynamic Grid Column Selector (3 / 4 / 5 Columns) & Sidebar Control** | Добавлен интерактивный селектор колонок сетки (3, 4, 5 колонок в ряд) в боковую панель «Библиотека» с сохранением выбора в `localStorage` (`pv_grid_columns`). По умолчанию возвращено классическое отображение в 3 колонки (`max-w-7xl`), при этом пользователи на широких мониторах или с низким зумом могут в 1 клик переключиться на 4 колонки (`max-w-[1680px]`) или 5 колонок (`max-w-[1920px]`) во всех 5 хабах (`Prompts`, `Skills`, `Git`, `Commands`, `Bookmarks`). |
| `86c101a` | 2026-08-26 | **🖥️ Responsive 4-Column Grid & Expanded Layout for Wide / Zoomed Screens** | Расширен максимальный контейнер приложения с 1280px до `max-w-[1680px]` и внедрена адаптивная 4-колоночная сетка (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4`) во все хабы (`Skills`, `Prompts`, `Commands`, `Git`, `Bookmarks`). |
