...
async function startServer() {
  ...
  app.post("/api/favorites/toggle", authenticate, async (req, res) => {
    try {
      ...
      if (!["prompt", "skill", "git_project", "command", "bookmark"].includes(itemType)) {
        return res.status(400).json({ message: "itemType должен быть prompt, skill, git_project, command или bookmark" });
      }
  ...
  app.get("/api/bookmarks", authenticate, async (req, res) => {
    try {
      ...
      const { data: favData } = await supabase
        ...
        .select("item_id")
        .eq("user_id", user.uid)
        .eq("item_type", "bookmark");

      const favIds = new Set((favData || []).map((f: any) => f.item_id));
      const rows = (data || []).map((row: any) => bookmarkFromDb(row, favIds.has(row.id)));
  ...
  app.delete("/api/bookmarks/:id", authenticate, async (req, res) => {
    try {
      ...
      await supabase.from("user_favorites").delete()
        .eq("item_id", id).eq("item_type", "bookmark");
