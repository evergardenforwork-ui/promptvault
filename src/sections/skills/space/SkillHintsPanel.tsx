    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="w-full max-w-xl bg-zinc-900 border border-zinc-700/60 rounded-2xl shadow-2xl shadow-black/60 flex flex-col max-h-[85vh] overflow-hidden"
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-black text-white">Подсказки</h2>
                <p className="text-[11px] text-zinc-500 truncate max-w-[260px]">{skillTitle}</p>
              </div>
            </div>
            <button
              onClick={onClose}
              className="p-2 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-xl transition-all cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {loading ? (
              ...
              <div className="text-center py-12 space-y-3">
                ...
                <p className="text-sm font-medium text-zinc-400">Нет подсказок</p>
              ...
              <AnimatePresence initial={false}>
                {hints.map(hint => (
                  <motion.div
                    ...
                    className="group relative bg-zinc-800/50 border border-zinc-700/40 hover:border-zinc-600/60 rounded-xl p-4 transition-all"
                  >
                    {/* Hint header */}
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <Pin className="w-3 h-3 text-amber-400 shrink-0 mt-0.5" />
                        <span className="text-sm font-bold text-white truncate">{hint.title}</span>
                      </div>
                      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        {canDeleteHint(hint) && (
                          <button
                            ...
                            className="p-1.5 text-zinc-600 hover:text-red-400 hover:bg-red-950/30 rounded-lg transition-all cursor-pointer disabled:opacity-50"
                            title="Удалить"
                          >
                            {deletingId === hint.id
            ...
            <AnimatePresence>
              {showForm && (
                ...
                >
                  <div className="bg-zinc-800/80 border border-violet-500/30 rounded-xl p-4 space-y-3">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-zinc-900 border border-zinc-700 focus:border-violet-500 rounded-xl text-sm text-white placeholder-zinc-600 outline-none transition-colors"
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-3 py-2 bg-zinc-900 border border-zinc-700 focus:border-violet-500 rounded-xl text-sm text-white placeholder-zinc-600 outline-none resize-none transition-colors leading-relaxed"
                      onKeyDown={e => {
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-zinc-600">Ctrl+Enter — сохранить</span>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => { setShowForm(false); setFormTitle(''); setFormText(''); }}
                          className="px-3 py-1.5 text-xs text-zinc-400 hover:text-white hover:bg-zinc-700 rounded-lg transition-all cursor-pointer"
                        >
                          Отмена
                        </button>
                        <button
            ...
            <button
              ...
              className={cn(
                ...
                showForm
                  ...
                  : 'bg-violet-600/10 text-violet-400 border-violet-500/30 hover:bg-violet-600/20 hover:text-violet-300'
              )}
            >
              {showForm ? (
